  ...
  foreach ($obtener_avance as $key_o => $value_o) {
    ...
    if ($value_o["ILOCALIZ"] == $value['estilo']) {
      ...
      if ($value['VoBo_confeccion'] != $proceso) {
        $actualizar_pedido_cadena .= "UPDATE PEDIDOS SET VoBo_confeccion = '$proceso'  WHERE clave = $clave;";
        ...
        if ($orden != '') {
          $actualizar_pedido_cadena .= "INSERT IGNORE INTO ORDENES_CONFECCION_PEDIDOS (orden_confeccion, clave_pedido, id_cliente) VALUES ('$orden', '$clave', '$cliente');";
        }
      }
    }
  }
